...
class _FingerprintHomeState extends State<FingerprintHome> {
  List<Map<String, dynamic>> fingerprints = [];
  ...
  Future<void> _loadFingerprints() async {
    final data = await DBHelper.getFingerprints();
  ...
  Future<void> _addFingerprint() async {
    final template = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => const RegisterNewFingerPrint(userId: 1),
      ),
    );
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            child: ListTile(
              title: Text(fp['name']),
              subtitle: Text("ID: ${fp['id']}"),
              trailing: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  IconButton(
                    icon: const Icon(Icons.verified_user, color: Colors.green),
                    onPressed: () => _verifyFingerprint(fp['template'], fp['name']),
                  ),
                  IconButton(
                    icon: const Icon(Icons.delete, color: Colors.red),
                    onPressed: () => _deleteFingerprint(fp['id']),
                  ),
                ],
              ),
            ),
